...
contract FlovatarPack{
    ...
    access(all) entitlement WithdrawEnt
	...
	resource Pack: Public{ 
		...
		access(account)
		let components: @[FlovatarComponent.NFT]
		...
		init(components: @[FlovatarComponent.NFT], randomString: String, price: UFix64, sparkCount: UInt32, series: UInt32, name: String){ 
			...
			FlovatarPack.totalSupply = FlovatarPack.totalSupply + 1
	...
	resource Collection: CollectionPublic{ 
		...
		access(account)
		let ownedPacks: @{UInt64: FlovatarPack.Pack}
		...
		view fun getPrice(id: UInt64): UFix64{ 
			let pack: &FlovatarPack.Pack = (&self.ownedPacks[id])!
			return pack.price
		}
		...
		fun purchaseDapper(tokenId: UInt64, recipientCap: Capability<&{FlovatarPack.CollectionPublic}>, buyTokens: @{FungibleToken.Vault}, signature: String, expectedPrice: UFix64){ 
			...
			pre{ 
				...
				buyTokens.isInstance(Type<@FlowUtilityToken.Vault>()):
					"Vault not of the right Token Type"
			}
		...
		fun purchaseWithDust(tokenId: UInt64, recipientCap: Capability<&{FlovatarPack.CollectionPublic}>, buyTokens: @{FungibleToken.Vault}, signature: String){ 
			...
			pre{ 
				...
				self.getPrice(id: tokenId) <= buyTokens.balance:
					"Not enough tokens to buy the Pack!"
				buyTokens.isInstance(Type<@FlovatarDustToken.Vault>()):
					"Vault not of the right Token Type"
			}
	...
	fun getPacks(address: Address): [UInt64]?{ 
		...
		if let packCollection = account.capabilities.borrow<&FlovatarPack.Collection>(FlovatarPack.CollectionPublicPath){ 
	...
	init(){ 
		...
		self.totalSupply = 0
		self.account.storage.save<@FlovatarPack.Collection>(
			<-FlovatarPack.createEmptyCollection(ownerVault: wallet),
			to: FlovatarPack.CollectionStoragePath
		)
		var capability_1 =
			self.account.capabilities.storage.issue<&{FlovatarPack.CollectionPublic}>(
				FlovatarPack.CollectionStoragePath
			)
		self.account.capabilities.publish(capability_1, at: FlovatarPack.CollectionPublicPath)
