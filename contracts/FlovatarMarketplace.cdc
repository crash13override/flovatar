...
contract FlovatarMarketplace{ 
	...
	access(all)
	event FlovatarForSale(id: UInt64, price: UFix64, address: Address)
	
	access(all)
	event FlovatarComponentForSale(id: UInt64, price: UFix64, address: Address, mint: UInt64)
	
	// Event that is emitted when the price of an NFT changes
	access(all)
	event FlovatarPriceChanged(id: UInt64, newPrice: UFix64, address: Address)
	
	access(all)
	event FlovatarComponentPriceChanged(id: UInt64, newPrice: UFix64, address: Address)
	
	// Event that is emitted when a token is purchased
	access(all)
	event FlovatarPurchased(id: UInt64, price: UFix64, from: Address, to: Address)
	
	access(all)
	event FlovatarComponentPurchased(id: UInt64, price: UFix64, from: Address, to: Address)
	...
	access(all)
	event FlovatarSaleWithdrawn(tokenId: UInt64, address: Address)
	
	access(all)
	event FlovatarComponentSaleWithdrawn(tokenId: UInt64, address: Address)
	...
	resource SaleCollection: SalePublic{ 
		...
		access(contract)
		let flovatarForSale: @{UInt64: Flovatar.NFT}
		
		access(contract)
		let flovatarComponentForSale: @{UInt64: FlovatarComponent.NFT}
		
		// Dictionary of the prices for each NFT by ID
		access(contract)
		let flovatarPrices:{ UInt64: UFix64}
		
		access(contract)
		let flovatarComponentPrices:{ UInt64: UFix64}
	...
	// when retrieved using the contract helper methods outside the collection.
	access(all)
	struct FlovatarComponentSaleData{ 
		access(all)
		let id: UInt64
		...
		access(all)
		let metadata: FlovatarComponent.ComponentData
		
		init(id: UInt64, price: UFix64, metadata: FlovatarComponent.ComponentData){ 
			self.id = id
			self.price = price
			self.metadata = metadata
		}
	...
	fun getFlovatarSales(address: Address): [FlovatarSaleData]{ 
		...
		if let saleCollection = account.capabilities.borrow<&FlovatarMarketplace.SaleCollection>(FlovatarMarketplace.CollectionPublicPath){ 
			...
			for id in saleCollection.getFlovatarIDs(){ 
				...
				saleData.append(FlovatarSaleData(id: id, price: price!, metadata: (flovatar!).getMetadata(), accessoryId: (flovatar!).getAccessory(), hatId: (flovatar!).getHat(), eyeglassesId: (flovatar!).getEyeglasses(), backgroundId: (flovatar!).getBackground()))
			}
		}
		return saleData
	}
	...
	fun getFlovatarComponentSales(address: Address): [FlovatarComponentSaleData]{ 
		...
		if let saleCollection = account.capabilities.borrow<&FlovatarMarketplace.SaleCollection>(FlovatarMarketplace.CollectionPublicPath){ 
			...
			for id in saleCollection.getFlovatarComponentIDs(){ 
				...
				saleData.append(FlovatarComponentSaleData(id: id, price: price!, metadata: FlovatarComponent.ComponentData(id: id, templateId: (flovatarComponent!).templateId, mint: (flovatarComponent!).mint)))
			}
		}
	...
	fun getFlovatarSale(address: Address, id: UInt64): FlovatarSaleData?{ 
		...
		if let saleCollection = account.capabilities.borrow<&FlovatarMarketplace.SaleCollection>(FlovatarMarketplace.CollectionPublicPath){ 
			...
			if let flovatar = saleCollection.getFlovatar(tokenId: id){ 
				let price = saleCollection.getFlovatarPrice(tokenId: id)
				return FlovatarSaleData(id: id, price: price!, metadata: flovatar.getMetadata(), accessoryId: flovatar.getAccessory(), hatId: flovatar.getHat(), eyeglassesId: flovatar.getEyeglasses(), backgroundId: (flovatar!).getBackground())
			}
		}
	...
	fun getFlovatarComponentSale(address: Address, id: UInt64): FlovatarComponentSaleData?{ 
		...
		if let saleCollection = account.capabilities.borrow<&FlovatarMarketplace.SaleCollection>(FlovatarMarketplace.CollectionPublicPath){ 
			...
			if let flovatarComponent = saleCollection.getFlovatarComponent(tokenId: id){ 
				let price = saleCollection.getFlovatarComponentPrice(tokenId: id)
				return FlovatarComponentSaleData(id: id, price: price!, metadata: FlovatarComponent.ComponentData(id: id, templateId: (flovatarComponent!).templateId, mint: (flovatarComponent!).mint))
			}
		}
